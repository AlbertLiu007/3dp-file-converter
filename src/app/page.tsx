  ...
  useEffect(() => {
    async function clearLegacyModelCache() {
      try {
        ...
        // Cache cleanup is best-effort; the page should still open as an empty converter.
      }
    }
    void clearLegacyModelCache();
  }, []);
        ...
        <aside className="grid gap-2">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            >
              ...
              {isConverting ? t.converting : isScaledExport && currentFile?.format === targetFormat ? t.scaleOnly : isScaledExport ? t.convertAndScale : t.convert}
            </button>
            {downloadFile ? (
              <button
                type="button"
